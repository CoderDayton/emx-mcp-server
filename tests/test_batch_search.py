...
class TestBatchSearch:
    ...
    def populated_store(self, vector_store, encoder):
        ...
        for start_idx in range(0, len(sample_texts), batch_size):
            ...
            metadata = [{"text": text, "index": start_idx + i} for i, text in enumerate(batch_texts)]
    ...
    def test_batch_search_basic(self, populated_store, encoder):
        ...
        for event_ids, distances, metadata in results:
            ...
            assert all(isinstance(d, (int, float)) for d in distances), "Distances are numeric"
    
    def test_batch_search_empty_queries(self, populated_store, encoder):
        """Test batch search with empty query list."""
        store, _ = populated_store
        
        results = store.search_batch(np.array([], dtype=np.float32).reshape(0, 384), k=5)
    ...
    def test_batch_search_relevance(self, populated_store, encoder):
        ...
        for i in range(len(distances) - 1):
            assert distances[i] <= distances[i+1], "Results should be sorted by distance"
        ...
        matches = sum(any(kw in text.lower() for kw in neural_keywords) for text in retrieved_texts)
    ...
    def test_batch_search_consistency(self, populated_store, encoder):
        ...
        for batch_res, seq_res in zip(batch_results, sequential_results):
            ...
            np.testing.assert_array_equal(batch_ids, seq_ids, 
                                         err_msg="Batch and sequential should return same IDs")
            np.testing.assert_allclose(batch_dists, seq_dists, rtol=1e-5,
    ...
    def test_recommended_batch_size_scaling(self, tmp_path, encoder):
        ...
        small_store = VectorStore(dimension=384, storage_path=str(tmp_path / "small"), use_gpu=False)
        assert small_store.get_recommended_batch_size() == 32, "Empty index should recommend batch=32"
        ...
        assert small_store.get_recommended_batch_size() == 32, "Small index recommends 32"
    ...
    def test_nlist_formula_options(self, tmp_path):
        ...
        for formula in formulas:
            ...
            n_vectors = 10000
            optimal_raw = store._calculate_optimal_nlist(n_vectors, formula=formula)
            ...
            assert optimal_raw == expected, f"{formula} should calculate correctly with bounds"
    ...
    def test_get_info_includes_batch_metadata(self, populated_store):
        ...
        assert "recommended_batch_size" in info, "Info should include recommended_batch_size"
        assert info["recommended_batch_size"] in [32, 64, 128], "Batch size should be valid"
    
    def test_batch_search_with_untrained_index(self, tmp_path, encoder):
        """Test batch search behavior with untrained index."""
        store = VectorStore(dimension=384, storage_path=tmp_path / "untrained", use_gpu=False)
        
        # Add vectors (not enough to trigger training)
        embeddings = encoder.encode_batch([["test", "text", "1"], ["test", "text", "2"]])
