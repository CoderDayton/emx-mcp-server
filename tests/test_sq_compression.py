...
class TestSQCompression:
    ...
    def test_sq_index_creation_and_training(self, temp_dir):
        ...
        assert info["index_type"] in ["IVFSQ", "SQ"]  # Could be either depending on nlist
    ...
    def test_sq_memory_reduction(self, temp_dir):
        ...
        assert compression_ratio > 3.5, f"Insufficient compression: {compression_ratio:.1f}x (expected ~4x)"
    ...
    def test_sq_flat_vs_ivf_selection(self, temp_dir):
        ...
        # Should be flat SQ for small datasets
        assert info_small["index_type"] == "SQ"
        ...
        # Should be IVF+SQ for large datasets
        assert info_large["index_type"] == "IVFSQ"
    ...
    def test_sq_cosine_similarity(self, temp_dir):
        ...
        # Should find similar vectors with high cosine similarity (distances close to 1.0)
        # Since we're using Inner Product with normalized vectors, high similarity = high distance
        assert len(ids) > 0
        assert all(d > 0.5 for d in distances), f"Low cosine similarity: {distances}"
        
        # The top result should be very similar
        assert distances[0] > 0.8, f"Top result not similar enough: {distances[0]:.3f}"