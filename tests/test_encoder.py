...
class TestEmbeddingEncoder:
    ...
    @pytest.mark.asyncio
    async def test_encode_individual_tokens(self, encoder, sample_tokens):
        """Test encoding individual tokens."""
        tokens = sample_tokens["short_sequence"][:3]  # First 3 tokens
        
        embeddings = encoder.encode_individual_tokens(tokens)
        
        # Check shape
        assert embeddings.shape == (len(tokens), encoder.dimension)
        assert embeddings.dtype == np.float32
        
        # Check that embeddings are not all zeros or NaN
        assert not np.allclose(embeddings, 0)
        assert not np.isnan(embeddings).any()
        assert not np.isinf(embeddings).any()
        
        # Check normalization (sentence-transformers normalizes embeddings)
    ...
    async def test_semantic_consistency(self, encoder):
        ...
        for texts in similar_texts:
            embeddings = encoder.encode_individual_tokens(texts)
    ...
    async def test_encode_tokens_with_context_progressive(self, encoder, sample_tokens):
        ...
        individual_embeddings = encoder.encode_individual_tokens(tokens)
