...
def test_temporal_chain_with_uuid_event_ids(tmp_path):
    ...
    for i, (from_id, to_id) in enumerate(temporal_links):
        ...
        assert from_id == expected_from, (
            f"Link {i}: from_event mismatch. "
            f"Got {from_id}, expected {expected_from}"
        )
        assert to_id == expected_to, (
            f"Link {i}: to_event mismatch. "
            f"Got {to_id}, expected {expected_to}"
        )
    ...
    assert db_event_ids == event_ids, (
        "Event IDs in graph DB don't match created events"
    )
...
def test_no_temporal_links_on_first_event(tmp_path):
    ...
    # Create single event
    result = manager.add_event(["first", "event"], embeddings=None, metadata={})
